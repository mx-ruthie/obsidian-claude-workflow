...
1. Use the Notion MCP to fetch the page — get the title, a 2-3 sentence summary of what it covers, and the URL. *(Skip if Notion MCP isn't configured — ask the user for a one-line summary instead.)*
2. Append to today's daily note (`YOUR_VAULT_PATH/Daily notes/DD-MM-YYYY.md`) under `## Docs & Decisions` (create the section if needed):
   ```
...
2. Append to today's daily note under `## Docs & Decisions`:
   ```
   - [[TICKET-ID]] Title — brief summary of what the ticket is about. [Linear](url)
   ```
