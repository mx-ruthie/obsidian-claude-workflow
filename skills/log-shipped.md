...
2. Use `gh` CLI to list PRs merged today by the user: `gh pr list --author "@me" --state merged --search "merged:>=TODAY" --json number,title,url,mergedAt` (replace TODAY with today's date in YYYY-MM-DD format)
3. For each shipped item, write a one-line entry in this format:
   - `- [[LINEAR-ID]] Title — one sentence of what it did or what problem it solved. PR: #number` (omit PR line if no linked PR)
